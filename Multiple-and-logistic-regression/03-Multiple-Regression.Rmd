...
plot_ly(data = babies, z = ~bwt, x = ~gestation, y = ~age, opacity = 0.6) %>% 
  ...
  add_surface(x = ~x, y = ~y, z = ~plane, showscale = FALSE, cmax = 1, surfacecolor = 'grey', colorscale = 'grey') %>%
  layout(
    xaxis = list(
      range = c(300, 355)
      ),
...
data_space <- ggplot(data = mario_kart, aes(x = duration, y = startPr, color = totalPr)) +
  geom_point()
...
```{r}
# draw the 3D scatterplot
p <- plot_ly(data = mario_kart, z = ~___, x = ~___, y = ~___, opacity = 0.6) %>%
